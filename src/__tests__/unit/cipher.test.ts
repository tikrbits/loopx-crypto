...
function _encipher(
  ...
  if (rng.randomRange(0, 2)) ctx.update(Buffer.alloc(0));
...
function encipher(name: string, key: Buffer, iv: Buffer, data: Buffer) {
  const ctx = new c.Cipher(name);
  const gcm = name.endsWith('-GCM');
  return _encipher(ctx, gcm, key, iv, data);
}

function decipher(name: string, key: Buffer, iv: Buffer, data: Buffer) {
  const ctx = new c.Decipher(name);
  const gcm = name.endsWith('-GCM');
  return _decipher(ctx, gcm, key, iv, data);
}

function encipher2(name: string, key: Buffer, iv: Buffer, data: Buffer) {
  const ctx = new c.Cipher(name);
  const gcm = name.endsWith('-GCM');
...
function decipher2(name: string, key: Buffer, iv: Buffer, data: Buffer) {
  const ctx = new c.Decipher(name);
  const gcm = name.endsWith('-GCM');
...
function encipher3(name: string, key: Buffer, iv: Buffer, data: Buffer) {
  const ctx = new c.Cipher(name);
  const gcm = name.endsWith('-GCM');
...
function decipher3(name: string, key: Buffer, iv: Buffer, data: Buffer) {
  const ctx = new c.Decipher(name);
  const gcm = name.endsWith('-GCM');
...
function encipherInc(name: string, key: Buffer, iv: Buffer, data: Buffer) {
  const gcm = name.endsWith('-GCM');
  const ctx = new c.Cipher(name);
  ...
  if (gcm) assert.doesNotThrow(() => ctx.getAuthTag());
  else assert.throws(() => ctx.getAuthTag());
...
function decipherInc(name: string, key: Buffer, iv: Buffer, data: Buffer) {
  const gcm = name.endsWith('-GCM');
  const ctx = new c.Decipher(name);
...
function encipherRand(name: string, key: Buffer, iv: Buffer, data: Buffer) {
  const gcm = name.endsWith('-GCM');
  const ctx = new c.Cipher(name);
...
function decipherRand(name: string, key: Buffer, iv: Buffer, data: Buffer) {
  const gcm = name.endsWith('-GCM');
  const ctx = new c.Decipher(name);
      ...
      for (const id of alg.ids) {
        ...
        for (const [key_, iv_, data_, expect_] of vectors) {
          ...
          it(`should encrypt and decrypt ${hex} with ${id}`, () => {
            ...
            if (!gcm) {
              assert.bufferEqual(c.encrypt(id, key, iv, data), expect);
              assert.bufferEqual(c.decrypt(id, key, iv, expect), data);
            }
          });
        }
      }
